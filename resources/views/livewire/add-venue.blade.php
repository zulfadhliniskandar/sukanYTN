<div class="max-w-3xl mx-auto py-10">
    ...
    <div class="mb-8 border-b border-gray-200 pb-5">
        ...
        <p class="mt-2 text-sm text-slate-500">
            Register a new location for the tournament events.
        </p>
    </div>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
        <form wire:submit="save" class="p-8 space-y-6">
            <!-- Venue Name -->
            <div>
                <label for="name" class="block text-sm font-semibold text-slate-700 mb-2">Venue Name <span class="text-red-500">*</span></label>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    <label for="latitude" class="block text-sm font-semibold text-slate-700 mb-2">Latitude <span class="text-red-500">*</span></label>
                    <input type="text" id="latitude" wire:model="latitude" 
                        class="w-full rounded-lg border-slate-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 transition-colors bg-slate-50 focus:bg-white" 
                        placeholder="3.054320">
                    @error('latitude') <span class="mt-1 text-sm text-red-600 block">{{ $message }}</span> @enderror
                </div>
                <div>
                    <label for="longitude" class="block text-sm font-semibold text-slate-700 mb-2">Longitude <span class="text-red-500">*</span></label>
                    <input type="text" id="longitude" wire:model="longitude" 
                        class="w-full rounded-lg border-slate-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 transition-colors bg-slate-50 focus:bg-white" 
                        placeholder="101.691116">
            ...
            <!-- Location Link -->
            <div>
                <label for="location_link" class="block text-sm font-semibold text-slate-700 mb-2">Google Maps Link (Optional)</label>
                <div class="relative">
            ...
            <div>
                ...
                <textarea id="description" wire:model="description" rows="4" 
                    class="w-full rounded-lg border-slate-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 transition-colors bg-slate-50 focus:bg-white resize-none" 
            ...
            <div class="pt-4 flex items-center justify-end border-t border-slate-100">
                <a href="{{ route('dashboard') }}" wire:navigate class="px-6 py-2.5 text-sm font-semibold text-slate-600 hover:text-slate-900 transition-colors mr-4">
                    Cancel
                </a>
