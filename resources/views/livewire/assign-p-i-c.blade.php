<div class="max-w-2xl mx-auto py-10 px-4 sm:px-6 lg:px-8 relative">
    ...
    <!-- Back Navigation -->
    <div class="mb-6">
        <a href="{{ route('listPIC', $sport) }}" wire:navigate class="inline-flex items-center text-sm font-medium text-slate-500 hover:text-indigo-600 transition-colors">
    ...
    <div class="bg-white rounded-3xl shadow-xl shadow-slate-200/50 border border-slate-100 overflow-hidden">
        ...
        <div class="relative px-8 py-6 bg-gradient-to-br from-indigo-600 via-indigo-700 to-violet-800 overflow-hidden">
            ...
            <h2 class="text-2xl font-black text-white tracking-tight">Assign PIC to {{ $sport->name }}</h2>
            <p class="mt-1.5 text-indigo-100 text-sm">Select a user to assign as the Person In Charge for this sport.</p>
        </div>

        <form wire:submit="store" class="p-8 space-y-6">
            <div>
                <label for="sport_id" class="block text-sm font-bold text-slate-700 mb-2">Sport</label>
                <input type="text" id="sport_id" name="sport_id" disabled value="{{ $sport->name }}"
                    class="block w-full rounded-xl border-slate-200 bg-slate-150/70 py-3 text-slate-550 cursor-not-allowed text-sm font-bold bg-slate-100 border border-slate-200 text-slate-500">
            </div>

            <div>
                <label for="user_id" class="block text-sm font-bold text-slate-700 mb-2">PIC Name</label>
                ...
                @error('pic') <span class="mt-1.5 text-xs text-rose-600 block font-semibold">{{ $message }}</span> @enderror
            </div>
